...
+(function ($) {
  ...
  Affix.prototype.getState = function (
    ...
    if (offsetTop != null && this.affixed == "top")
      return scrollTop < offsetTop ? "top" : !1;
    if (this.affixed == "bottom") {
      if (offsetTop != null)
        return scrollTop + this.unpin <= position.top ? !1 : "bottom";
...
(function (factory) {
  ...
  $.fn.tilt = function (options) {
    ...
    const getValues = function () {
      ...
      const percentageX = this.mousePositions
        ? (this.mousePositions.x - left) / width
        : 0;
      const percentageY = this.mousePositions
        ? (this.mousePositions.y - top) / height
        : 0;
      const tiltX = (
      ...
      const angle = this.mousePositions
        ? Math.atan2(
...
(function (window, factory) {
  ...
  var cursorNodes = { TEXTAREA: !0, INPUT: !0, SELECT: !0, OPTION: !0 };
  var clickTypes = {
    radio: !0,
    checkbox: !0,
    button: !0,
...
(function (window, factory) {
  ...
  utils.extend(Flickity.defaults, { draggable: ">1", dragThreshold: 3 });
  Flickity.createMethods.push("_createDrag");
  var proto = Flickity.prototype;
  utils.extend(proto, Unidragger.prototype);
  proto._touchActionValue = "pan-y";
  var isTouch = "createTouch" in document;
  var isTouchmoveScrollCanceled = !1;
  ...
  var focusNodes = { INPUT: !0, TEXTAREA: !0, SELECT: !0 };
...
(function (window, factory) {
  ...
  utils.extend(Flickity.defaults, { pageDots: !0 });
...
(function (window, factory) {
  ...
  utils.extend(Flickity.defaults, { pauseAutoPlayOnHover: !0 });
...
function transFormLate(element, direction, gotoPos) {
  element.css({
    "-webkit-transform": "translate" + direction + "(" + gotoPos + "px)",
    "-moz-transform": "translate" + direction + "(" + gotoPos + "px)",
    "-ms-transform": "translate" + direction + "(" + gotoPos + "px)",
    "-o-transform": "translate" + direction + "(" + gotoPos + "px)",
    transform: "translate" + direction + "(" + gotoPos + "px)",
  });
}
...
function rotateTopArrow() {
  var switchPos = win.height() / 2,
    arrow = jQuery("#go-to-top");
  if (jQuery(".full-section").length) {
    if (switchPos <= win.scrollTop()) {
      arrow.removeClass("showDown");
      arrow.attr("href", "#");
    } else {
      arrow.addClass("showDown");
      arrow.attr("href", "#first-wave");
    }
  } else {
    if (switchPos <= win.scrollTop()) {
      arrow.fadeIn();
...
jQuery(document).ready(function ($) {
  ...
  rotateTopArrow();
  jQuery(".scrolltonext").click(function () {
    var goto = jQuery(this).attr("href");
    // console.log("Going to ", goto);
    if (goto == "#") {
      goto = "body";
    }
    jQuery("body,html").animate({ scrollTop: jQuery(goto).offset().top }, 800);
    return !1;
  });

  jQuery("#about-id").click(function () {
  ...
  jQuery("#tech-id").click(function () {
    jQuery("body,html").animate(
      { scrollTop: $("#tech-to-scroll").offset().top + 200 },
      1000
    );
  });

  jQuery("#work-id").click(function () {
    jQuery("body,html").animate(
      { scrollTop: $("#projects_slider").offset().top - 100 },
      1400
    );
  });

  jQuery("#exp-id").click(function () {
    jQuery("body,html").animate({ scrollTop: $("#exp").offset().top }, 1700);
  });

  jQuery("#hello-id").click(function () {
    jQuery("body,html").animate(
      { scrollTop: $("#hello").offset().top - 100 },
      2000
    );
  });

  jQuery("#nav-tech").click(function () {
    jQuery("#navi").toggleClass("active");
    jQuery("body").toggleClass("onHold");
  });

  jQuery("#nav-about").click(function () {
    jQuery("#navi").toggleClass("active");
    jQuery("body").toggleClass("onHold");
  });

  jQuery("#nav-work").click(function () {
    jQuery("#navi").toggleClass("active");
    jQuery("body").toggleClass("onHold");
  });

  jQuery("#nav-exp").click(function () {
    jQuery("#navi").toggleClass("active");
    jQuery("body").toggleClass("onHold");
  });

  jQuery("#nav-hello").click(function () {
    jQuery("#navi").toggleClass("active");
    jQuery("body").toggleClass("onHold");
  });

  function fadeOutOnTop(element) {
    if (element.length) {
      var eTop = element.offset().top - win.scrollTop();
      if (eTop <= 150) {
        element.addClass("nV fade");
      } else {
        if (element.hasClass("fade")) {
          element.removeClass("nV fade");
        }
      }
    }
  }
  if (jQuery(".js-tilt").length && !isTouchDevice()) {
    jQuery(".js-tilt").tilt({
      maxTilt: 20,
      perspective: 1000,
  ...
  jQuery("a.canvas-navi-bar, .overlay").click(function () {
    jQuery("#navi").toggleClass("active");
    jQuery("body").toggleClass("onHold");
    if (jQuery("body").hasClass("onHold")) {
      disableScroll();
    } else {
      enableScroll();
    }
  ...
  jQuery(".carousel.projects_slider-container").flickity({
    cellAlign: "left",
    prevNextButtons: !1,
    contain: !0,
  });
  jQuery(".image-slider").flickity({
    contain: !0,
    cellAlign: "center",
    prevNextButtons: !1,
    groupCells: "80%",
  ...
  jQuery(function () {
    var filterList = {
      init: function () {
        jQuery(".projekt-container").mixItUp({
          selectors: {
            target: ".projektDiv",
  ...
  $("#feel-the-wave-two").wavify({
    ...
    bones: 3,
    amplitude: 120,
    color: "rgba(242, 243, 247, .5)",
    speed: 0.2,
  });
  var allMods = jQuery(".ani");
  ...
  win.scroll(function (event) {
    ...
    fadeOutOnTop(jQuery(".bookmark"));
    fadeOutOnTop(jQuery(".bx-pager"));
    fadeOutOnTop(jQuery(".socialicons"));
    rotateTopArrow();
  });
  jQuery(".service").each(function (i, el) {
    var el = jQuery(el),
      containerWidth = Math.ceil(el.width());
    el.mousemove(function (e) {
      var parentOffset = jQuery(this).offset(),
        mousePosition = e.pageX - parentOffset.left,
        mouseDisToMid = mousePosition - containerWidth / 2,
        move = -20,
        gotoPos = (move / (containerWidth / 2)) * mouseDisToMid;
      transFormLate(jQuery("img", this), "X", gotoPos);
    });
    el.mouseleave(function () {
      jQuery("img", this).attr({ style: "" });
    });
  });
  jQuery("#site-background .layer-wrapper > div").each(function (i, el) {
    var el = jQuery(el);
    function moveLayers(el) {
      if (el.hasClass("layer2")) {
        var depth = 100;
      } else {
        var depth = -100;
      }
      var move = (depth / jQuery("body").height()) * win.scrollTop();
      transFormLate(el, "Y", move);
    }
    moveLayers(el);
  ...
  if (jQuery("body").hasClass("error404")) {
    jQuery(".contact-cta").remove();
  }
});
jQuery("body").addClass("onHold");
disableScroll();
win.load(function () {
  jQuery
    .when(
      jQuery("#overlayer")
        .delay(2000)
        .queue(function (next) {
          jQuery(this).addClass("zoom").fadeOut("slow");
          next();
        })
    )
    .done(function () {
      enableScroll();
      jQuery("body").removeClass("onHold");
      jQuery(".ani").each(function (i, el) {
        var el = jQuery(el);
        if (el.visible(!0)) {
          el.removeClass("nV");
        }
